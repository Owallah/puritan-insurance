...
export function PaystackPaymentDialog({
  ...
  async function handlePay() {
    ...
    try {
      ...
      // ─────────────────────────────────────────────────────────────
      // INLINE PAYSTACK CHECKOUT (NO NEW TAB)
      // ─────────────────────────────────────────────────────────────

      const handler = (window as any).PaystackPop.setup({
        key: process.env.NEXT_PUBLIC_PAYSTACK_PUBLIC_KEY,
        email,
        amount: premiumOption.amount * 100,
        ...
        onClose: function () {
          setState("failed");
          setErrorMsg("Payment window closed before completion.");
        },
      });
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[440px] p-0 rounded-2xl">
        ...
        <div className="px-6 py-6 space-y-4">
          ...
          {/* INITIALIZING */}
          {state === "initializing" && (
            <Status title="Setting up payment..." />
          )}

          {/* PAYING */}
          {state === "paying" && (
            <Status title="Complete payment in popup..." />
          )}

          {/* VERIFYING */}
          {state === "verifying" && (
            <Status title="Verifying payment..." />
          )}

          {/* CONFIRMED */}
          {state === "confirmed" && (
            <Status title="Payment successful!" />
          )}

          {/* FAILED */}
          {state === "failed" && (
            <>
              <Status title={errorMsg || "Payment failed"} />
