...
const SUPPORT_PHONE = process.env.SUPPORT_PHONE  ?? "+254 700 000 000";
...
function formatDate(raw: string | null | undefined): string {
  ...
  // Daraja TransactionDate format: YYYYMMDDHHmmss
  if (/^\d{14}$/.test(raw)) {
    const y = raw.slice(0, 4), mo = raw.slice(4, 6), d = raw.slice(6, 8);
    const h = raw.slice(8, 10), mi = raw.slice(10, 12), s = raw.slice(12, 14);
    return new Date(`${y}-${mo}-${d}T${h}:${mi}:${s}+03:00`).toLocaleString("en-KE", {
...
function buildReceiptHtml(payment: PaymentRecord): string {
  ...
  const rows: [string, string][] = [
    ...
    ["Date & Time",         formatDate(payment.transaction_date)],
  ];
      ...
      <table width="600" cellpadding="0" cellspacing="0" role="presentation"
        ...
        <tr>
          <td style="background:#0a1849;padding:32px 40px;text-align:center;">
            ...
            <p style="margin:6px 0 0;font-size:12px;font-weight:600;letter-spacing:2px;
                      text-transform:uppercase;color:#e6c367;">
              Payment Receipt
            </p>
          </td>
        </tr>
          ...
          <td style="padding:0 40px;">
            <table width="100%" cellpadding="0" cellspacing="0" role="presentation"
              ...
              <tr style="background:${i % 2 === 0 ? "#f8faff" : "#eef1ff"};">
                <td style="padding:12px 20px;font-size:13px;font-weight:600;color:#374151;
                           width:45%;border-bottom:1px solid #dde2f0;">${label}</td>
                <td style="padding:12px 20px;font-size:13px;color:#0a1849;font-weight:700;
                           border-bottom:1px solid #dde2f0;">${value}</td>
              </tr>`).join("")}
            </table>
          </td>
        </tr>
 ...
 * Called from the M-Pesa callback route after a successful transaction.
 * Errors are logged but never thrown — a failed email must not disrupt the callback flow.
 */
export async function sendPaymentReceipt(payment: PaymentRecord): Promise<void> {
  ...
  if (error) console.error("[sendPaymentReceipt] Resend error:", error);
}
